...
namespace PaperTrails
{
    public sealed class SoundBank : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            string[] names={"coin","capture","cut","death","respawn","overtime","finish","roulette","unlock","tick","go"};
            for(int k=0;k<names.Length;k++)
            {
                int count=names[k]=="finish"?22050:names[k]=="go"?9000:7000;var samples=new float[count];
                for(int i=0;i<count;i++){float t=i/22050f,env=Mathf.Pow(1-i/(float)count,2);float f=names[k]=="death"?250-t*450:330+k*47+(names[k]=="coin"?t*1400:0)+(names[k]=="go"?t*1800:0);samples[i]=Mathf.Sin(2*Mathf.PI*f*t)*env*.45f;}
